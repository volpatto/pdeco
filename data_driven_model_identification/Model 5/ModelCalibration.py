...
"""
	...
	def __create_smc_code(self, param_names, bounds, sd_upper_bound, sd_shape, param_expression, init_cond_expression, draws, seed):
		...
		for i, param_name in enumerate(param_names):
			if bounds[i][0] == bounds[i][1]:
				code += param_name + """ = pm.Data('""" + param_name + """', """ + str(bounds[i][0]) + """)
			"""
			else:
			...
			"""
			# 	mu = 0.5*(bounds[i][0] + bounds[i][1])
			# 	sigma = abs(1.0 - bounds[i][0]/mu)
			# 	if mu < 0.0:
			# 		code += """BoundedNormal = pm.Bound(
			# 			pm.Normal,
			# 			upper = 0.0
			# 		)
			# """
	...
	def bayesian_calibration(self, bounds_perc = 0.01, sd_bound_perc = 0.1, draws = 2500, seed = None):
		...
		bounds = []
		for c_id, c in enumerate(np.append(coef[ind].flatten(), self.X0)):
			if c_id == 0:
				if c < 0.0:
					bounds.append([(1.0 - 1.0*bounds_perc)*c, (1.0 - 3.0*bounds_perc)*c])
				else:
					bounds.append([(1.0 - 3.0*bounds_perc)*c, (1.0 - 1.0*bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 2.0*bounds_perc)*(-9.715464592532921e-05), (1.0 - 1.5*bounds_perc)*(-9.715464592532921e-05)])
				# else:
				# 	bounds.append([(1.0 - 1.5*bounds_perc)*(-9.715464592532921e-05), (1.0 + 2.0*bounds_perc)*(-9.715464592532921e-05)])
			elif c_id == 1:
				bounds.append([c, c])
				# bounds.append([-0.007269048423168431, -0.007269048423168431])
			elif c_id == 2:
				if c < 0.0:
					bounds.append([(1.0 + 0.0*bounds_perc)*c, (1.0 - 2.0*bounds_perc)*c])
				else:
					bounds.append([(1.0 - 2.0*bounds_perc)*c, (1.0 + 0.0*bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 4.0*bounds_perc)*3.9189661209196945e-08, (1.0 - bounds_perc)*3.9189661209196945e-08])
				# else:
				# 	bounds.append([(1.0 - bounds_perc)*3.9189661209196945e-08, (1.0 + 4.0*bounds_perc)*3.9189661209196945e-08])
			elif c_id == 3:
				bounds.append([c, c])
				# if c < 0.0:
				...
				# 	bounds.append([(1.0 - bounds_perc)*c, (1.0 + bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 4.0*bounds_perc)*2713.7232754553243, (1.0 - 0.5*bounds_perc)*2713.7232754553243])
				# else:
				# 	bounds.append([(1.0 - 0.5*bounds_perc)*2713.7232754553243, (1.0 + 4.0*bounds_perc)*2713.7232754553243])
				# bounds.append([2713.7232754553243, 2713.7232754553243])
			elif c_id == 4:
				bounds.append([c, c])
				# bounds.append([4.147055452699332, 4.147055452699332])
		sd_upper_bound = 1000 # sd_bound_perc*max(self.X.min(), self.X.max(), key = abs)
		...
		self.calibration_param_names = np.delete(self.calibration_param_names, np.where(self.calibration_param_names == "c4"))
		self.calibration_param_names = np.delete(self.calibration_param_names, np.where(self.calibration_param_names == "x0"))
		self.calibration_param_names = np.delete(self.calibration_param_names, np.where(self.calibration_param_names == "y0"))
