...
"""
	...
	def bayesian_calibration(self, bounds_perc = 0.01, sd_bound_perc = 0.1, draws = 2500, seed = None):
		...
		for c_id, c in enumerate(np.append(coef[ind].flatten(), self.X0)):
			if c_id == 0:
				if c < 0.0:
					...
					bounds.append([(1.0 - bounds_perc)*c, (1.0 + bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 2.0*bounds_perc)*(-9.028352501540535e-05), (1.0 - 2.5*bounds_perc)*(-9.028352501540535e-05)])
				# else:
				# 	bounds.append([(1.0 - 2.5*bounds_perc)*(-9.028352501540535e-05), (1.0 + 2.0*bounds_perc)*(-9.028352501540535e-05)])
			elif c_id == 1:
				if c < 0.0:
					bounds.append([(1.0 - bounds_perc)*c, (1.0 - 3.0*bounds_perc)*c])
				else:
					bounds.append([(1.0 - 3.0*bounds_perc)*c, (1.0 - bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 3.0*bounds_perc)*3.677893638863118e-08, (1.0 - bounds_perc)*3.677893638863118e-08])
				# else:
				# 	bounds.append([(1.0 - bounds_perc)*3.677893638863118e-08, (1.0 + 3.0*bounds_perc)*3.677893638863118e-08])
			elif c_id == 2:
				bounds.append([c, c])
				# bounds.append([-0.0016010265545714783, -0.0016010265545714783])
			elif c_id == 3:
				bounds.append([c, c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + bounds_perc)*c, (1.0 - bounds_perc)*c])
				...
				# 	bounds.append([(1.0 - bounds_perc)*c, (1.0 + bounds_perc)*c])
				# if c < 0.0:
				# 	bounds.append([(1.0 + 4.0*bounds_perc)*2681.4743756420753, (1.0 - bounds_perc)*2681.4743756420753])
				# else:
				# 	bounds.append([(1.0 - bounds_perc)*2681.4743756420753, (1.0 + 4.0*bounds_perc)*2681.4743756420753])
				# bounds.append([2681.4743756420753, 2681.4743756420753])
			elif c_id == 4:
				bounds.append([c, c])
				# bounds.append([4.1470600152530155, 4.1470600152530155])
		...
		self.calibration_param_names = np.delete(self.calibration_param_names, np.where(self.calibration_param_names == "c8"))
